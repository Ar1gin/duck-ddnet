	...
	int ClientId)
{
	...
	if(g_Config.m_DcGrenadePath && Player.m_Weapon == WEAPON_GRENADE && g_Config.m_ClShowOthersAlpha > 0)
	{
		...
		CProjectile *proj = new CProjectile(&GameClient()->m_PredictedWorld, WEAPON_GRENADE, ClientId, OldGPos, Direction, projlifetime, false, false, 0, LAYER_GAME, -1);
	...
	if(Local && g_Config.m_DcLaserPath && Player.m_Weapon == WEAPON_LASER && (GameClient()->m_GameWorld.m_WorldConfig.m_IsFNG || AlwaysRenderHookColl || RenderHookCollPlayer) && g_Config.m_ClShowOthersAlpha > 0)
	{
		float Alpha = GameClient()->IsOtherTeam(ClientId) ? g_Config.m_ClShowOthersAlpha / 100.0f : 1.0f;
		Alpha *= (float)g_Config.m_ClHookCollAlpha / 100;
